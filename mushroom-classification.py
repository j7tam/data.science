...
# MILDER IMG AUGMENTATION FOR RESNET-18. UNCOMMENT AND RUN IF TESTING RESNET 18
...
train_tfms = transforms.Compose([
    ...
    transforms.RandomHorizontalFlip(p=0.5),
    transforms.RandomRotation(degrees=15),
    transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.05),  # --- CHANGED --- stronger jitter
    RandAugment(num_ops=2, magnitude=9),        # --- NEW --- learned augmentation policy
    transforms.ToTensor(),
    transforms.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD),
    transforms.RandomErasing(p=0.25),           # --- NEW --- CutOut-style regularization
])
...
model = timm.create_model(
    ...
    pretrained=True,
    num_classes=len(train_classes)  # using your existing train_classes
)
...
# ============================================================
train_ds = MushroomDataset(train, transform=train_tfms, class_to_idx=train_class_to_idx, classes=train_classes)
val_ds   = MushroomDataset(val,   transform=val_tfms,          class_to_idx=train_class_to_idx, classes=train_classes)
test_ds  = MushroomDataset(test,  transform=val_tfms,          class_to_idx=train_class_to_idx, classes=train_classes)
...
get_ipython().run_line_magic('pip', 'install grad-cam')
